...
今までの記事で紹介していたjQueryでは、「slick等でスライドショーを動かす」、「ボタンを任意の箇所までスクロール」等、**静的なページに一部動きを付ける**のが主な役割でした。対してReactは、SNS、ECサイトのような**ユーザーの操作・サーバーの応答によって変化するデータに応じて画面を最適に書き換えるWEBアプリケーションの制作**に特化しています。また、モバイルアプリの制作にもReactの技術が使用されているものもあります。

### jsx記法によるUI

jsxとはJavaScriptでUI記述していく構文です。
...
HTMLをパーツ化し、状態(データ)を持たせることが可能です。

### 宣言的なUI

今までにjQueryの記事で紹介したような「ボタンを押したらこの要素を書き換える(処理)」という処理(命令型)ではなく、「データがこうなったら画面はこう見える」というルール(宣言型)を決めるだけです。
...
多くの場合、Reactを単体で使用するケースは少なく、Next.jsやReactNativeというフレームワーク内でReactを使用します。

React単体での使用の場合は`create-react-app`を使用します。

```shell
# my-appプロジェクトを作成、プロジェクトのフォルダへ移動
