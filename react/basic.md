...
**★直接的に値を変更できる**  
stateはその値を更新するために専用の関数を使用する(イミュータブルに扱う)必要があり、値の更新が反映されるのも次レンダーでした。refはただのJavaScriptオブジェクトなので、`current`プロパティに入っている値を直接変更でき、その変更は即反映されます。
...
**★DOM操作をしない**  
DOMの変更は飽くまでReactの仕事です。refを使って勝手にReactが管理すべきDOMの変更を行うと、React側が把握できないため、実行時エラーになります。
