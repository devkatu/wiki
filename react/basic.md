...
stateを更新すると、再レンダーが発生しますが、いくつか注意する点があります。
...
先の例の`setCount`もそうですが、次のような例ではどうでしょう。
...
`setCount`によって、再レンダーが要求されますが、そのレンダーが発生するまではstateの値は更新されないため、上記の例では`count`の値は更新されず、`0`と表示されました。

上記のように更新された値を使って何らかの処理をしたい場合は、state更新と同様の処理を行う変数を用いるなどの対策をします。
