...
また、propsとしてイベントハンドラを渡す場合は、`onClick={toggleLike()}`とはせずに、`onClick={toggleLike}`のように、`()`を付けないようにしましょう。
...
または、イベントハンドラでの処理が短いものであれば、インラインで即時関数として`onClick={() => alert()}`のような書き方もアリです。
