...
なお、**stateの更新は、state更新関数のみで行い、`state = true`のような更新はしてはいけません。** これはreact側でstateの更新を検出できなくなってしまうためです。読取専用の変数のように扱うようにしましょう(イミュータブルともいう)
...
また、ほとんどの場合、配列やオブジェクトの一部を更新する事になります。「今のstateに対する変更」となるので先に述べた「stateをどのように更新するか定義した関数」でstateを更新します。

**既存のstateをスプレッド構文でコピーして、新しい配列やオブジェクトを作成し、それを新しいstateとしてセット**するのがよく使うパターンとなります。
...
ただしスプレッド構文は**浅いコピー（shallow copy）**なので、配列やオブジェクトの最上位階層のみをコピーするため、深くネストされたオブジェクトの更新には注意が必要です。

複雑な場合は`Immer`といったライブラリを利用して、変更したいプロパティのみを指定して更新することもできます。

また、配列メソッドの中には`push`、`pop`等の、**既存の配列を変更してしまうメソッド(破壊的メソッド)があるので、これをstateに対して使ってはいけません。**
