...
- 再レンダリングが発生したとき  
  →一度クリーンアップ処理を行った後に、再度エフェクトが実行されます
- コンポーネントが削除されたとき
...
**★第二引数にはエフェクト実行を制御する為の依存配列を渡します**  
`useEffect`処理の中で使用している、再レンダー時に変化する可能性のある変数(リアクティブな変数ともいう)を配列として渡します。指定した変数が前回レンダーと今回レンダーで同じ(`Object.is()`を用いて判定している)場合はエフェクトの実行はスキップされます。

- 第二引数に空の配列 `[]` を渡すと、初回マウント時のみ実行されます。
- 第二引数を省略すると、毎回のレンダー後に実行されます。




エフェクトの異なる部分を異なる条件で再実行したい。
依存値の最新の値を読み取りたいだけで、その変化に「反応」したいわけではない。
依存値の型がオブジェクトや関数であるために、意図せずに頻繁に変更される。
