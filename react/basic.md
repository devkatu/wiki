...
アコーディオンメニューの開閉や、タブメニューの切り替えなど、ユーザーの操作に応じて画面の表示内容を変えたいとき、Reactでは**state(状態)**を使用します。

propsが「外部から渡されるデータ」だったのに対し、state は「コンポーネント自身が内部で保持するデータ」のことです。**このstateに変更を与えてやることで、再びレンダリング(表示する要素の再計算)が発生し、画面の表示内容が更新されます。**
...
先に述べたように、関数はデータを保持しておくことができないので、`useState`にてコンポーネントのデータを保持する領域を作ってもらいます。

`useState`は、**引数にそのstateの初期値を渡し、返り値として現在の値と更新するための関数を受け取ります**。

`[現在state, state更新関数] = useState(state初期値)`

返り値は配列ですが、例のように分割代入とします。

また、stateを`value`とした場合、state更新関数の方は`setValue`のように頭に`set`を付けるのが一般的です。





stateを扱う際に最も重要なルールは、**イミュータブル（不変）に扱う**ことです。
...
引数名には、上記のように`prev`を使ったり、`count`なら`c`といった変数の頭文字を使ったりするのが一般的です。
