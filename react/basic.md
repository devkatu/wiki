...
Reactにおける「state（状態）」は、コンポーネントが保持するデータのことです。
本来、JavaScriptの関数そのものは状態を保存しておくことができません。コンポーネント関数はレンダリングの都度呼び出され、その中のローカル変数はレンダリングが終わると破棄されてしまいます。
しかしReactでは、React本体側にコンポーネント専用の変数領域を用意してもらうことで、そこにデータを保存し、あたかも関数が状態を持っているように見せかけることができます。
