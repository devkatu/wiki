...
`useEffect`処理の中で使用している、**props、state、及びこれらから計算した値**を配列として渡します。
...
`memo`と組み合わせて、子コンポーネントにpropsとして関数を渡す際の不要な再レンダリングを防ぐのに役立ちます。

#### 使い方

これも使い方は非常に簡単で、**メモ化したい関数を`useCallback`の第一引数に渡す**だけです。第二引数にはメモ化された関数を再度計算し直すのに必要な依存配列を渡します。
...
メモ化した関数は、そのまま子コンポーネントのpropsに渡します。渡した関数はメモ化されているので子コンポーネントは毎回のレンダリングを防ぐことができます。
...
よくある誤りとしてメモ化した関数をさらに即時関数を作ってその中で実行するものがあります。これだと再レンダリング時に結局新しい関数が作られてしまうのでメモ化の効果が無くなります。
