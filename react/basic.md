...
アプリの規模が大きくなってくると、stateが肥大化していき、更新処理も複雑化していきます。

複雑化していくstateの更新処理が、複数の箇所で行われると、バグの原因となりやすいです。
...
コンポーネント内でstateを更新するための多数のロジックが書かれています。

コンポーネントを「どう表示するか」に集中したいのに、「データの加工」の事が邪魔してコード全体の見通しが悪い
ここにさらに追加の処理を加えたくなったとしたらますます悪化します

このstateの更新ロジックを、コンポーネントの外へ追い出す事ができます。
