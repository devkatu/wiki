...
※スプレッド構文に加えて、更新したいプロパティのみではだめな旨も記載
...
ただしスプレッド構文は**浅いコピー（shallow copy）**なので、配列やオブジェクトの最上位階層のみをコピーするため、深くネストされたオブジェクトの更新には注意が必要です。複雑な場合は`Immer`といったライブラリを利用して、変更したいプロパティのみを指定して更新することもできます。
