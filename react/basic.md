...
- state
  ...
  - props同様にイミュータブル(自分で勝手に書換しない)に扱う
  - setStateを行っても、今回レンダーの中ではstateの値は更新されず、次回レンダーで更新された値によるレンダーが行われる
  - 関数を渡した場合は「stateの値に対してこう変更してね」を定義するので複雑な更新も可能。
    - また、同じコールバック内で`setstate(value + 1)`を連続して実行してもその`value + 1`がセットされるだけだたが、`setState(v => v + 1)`とすると回数分`v+1`される。なお、この連続した`setState`はreactがバッチ処理として一括して行い、コールバック内の処理が全て終わってからstateを計算し、レンダーが発生する(setStateの都度レンダーが発生するわけではない)
    - また、オブジェクトの更新の時に既存のstateをスプレッド構文でコピーするのが良い。但し、shallowコピーなので注意。Immerなるライブラリで**変更したいプロパティのみ**を指定してsetStateすることも可能らしい
    - 配列のstateも同様。既存のstateをスプレッドでコピーしてsetStateする。それか`map``filter``slice`等を使ってやる。`pusu`等の破壊的メソッドはstateを書き換えてしまうのでNG
  - 更新用関数の引数は、対応するstate変数の頭文字を使って付ける、`prev`のようなプレフィックスを付けるのが一般的
  - stateの設計について…データベース構造の正規化同様バグを減らす為に重要
    - 常に同じタイミングで一緒に変更されるstateはオブジェクト等に纏める(`x`,`y`という個別のstateがあれば`{x, y}`)
    - パフォーマンスに影響するので、既存のprops,stateから計算できる情報をstateにしない(`firstName`,`lastName`がstateとしてあるなら`fullName`はstateではなく計算して出す等)。また、下記も既存のprops,stateから計算できるのであればやらないほうが良い
      - コンポーネント内でstateでの重複を避ける
      - 矛盾したstateにならないようにする
    - stateが深くネストされるのを避ける
  - 複数のコンポーネントでstateを共有したい場合はリフトアップする
  - reducerは、一言で言うとstate更新のライブラリみたいなもの(言い方あってる？)
    - 複雑なstate更新が複数箇所で行われる場合に便利。直接stateを更新するのではなく、`dispatch`でアクションを指定し、reducer関数側でアクションに対するstateの更新の仕方を定義する。「発生したこと」と「stateをどう更新するか」を分けられる。→関心の分離
    - シンプルなstate更新であればいらない(むしろreducerを入れることでコードが増える恐れもある)が、複雑なstate更新の際に効果を発揮。コード量が減って見通しが良くなるし、誤ったsetStateをする恐れがなくなる
...
先に述べたように、関数はデータを保持しておくことができないので、**該当コンポーネントで固有のデータを保持する領域を`useState`で作ってもらいます**。
...
**引数にそのstateの初期値を渡し、返り値として「現在のstateの値」と「stateを更新するための関数」を配列として受け取ります**。※返り値を分割代入で受け取っている。

この`useState`の呼び出しは**必ずコンポーネント関数内の最初に行い**、条件分岐、ループ内、ネストされた関数内では行いません。他にもコンポーネント内で必要なstateが出てきたら、同様に必要なだけの`useState`を追加して行くこととなります。

また、stateを`value`という名前にした場合、state更新関数の方は`setValue`のように`set`＋`state変数名`のようにするのが慣例となっています。

そして、state更新関数をイベントハンドラで実行するようにしてあげます。
...
現在stateは、propsでの扱いと同じく、波括弧`{}`を用いてJSX内で展開することで、表示する要素をコントロールすることとなります。
...
のように、「レンダー時のstate値を使って、次のstate値を返す関数」を渡してあげます。

これは次のケースのときに役立ちます。

- 一つのイベント内で複数回のstate更新をしたいとき
- 配列やオブジェクトの更新をしたいとき

一つのイベント内で複数回のstate更新をしたい場合は、次のようにしてもうまくいきません。
...
次のような関数を渡した場合はreactに思惑通り処理されます。
...
渡す関数の引数名には、慣例的に`count`なら`c`といったstate変数の頭文字を使ったり、`prev`を使います。
...
ただし、配列やオブジェクトの更新時は、そのstateを**イミュータブル（不変）**に扱わなければなりません。

要は`const`宣言されている変数のように、元のstateを変えてしまうようなことはしてはいけないということです。

例えば、配列のstateに対して`push`のような破壊的メソッドを使って直接書き換えてはいけません。必ず既存のstateをコピーして新しい配列やオブジェクトを作成し、それをセットします。
...
// OK: スプレッド構文でコピーして新しい配列を作成
setItems([...items, 'ぶどう']);
```

オブジェクトの更新時も同様に、既存のstateをスプレッド構文でコピーして、変更したいプロパティのみを上書きします。
（※ただしスプレッド構文は浅いコピー（shallow copy）となるため、深くネストされたオブジェクトの更新には注意が必要です。複雑な場合は`Immer`といったライブラリを利用して、変更したいプロパティのみを指定して更新することもあります）

#### stateの更新による再レンダー

再レンダーのトリガーとなる条件は、

- 該当のコンポーネントや、その祖先のコンポーネントのstateが更新された
- 親コンポーネントが再レンダリングされた
- useContextを使っているときに、そのデータが更新された



stateが変化すると、Reactはそのコンポーネントと、それに含まれる子コンポーネントを再レンダー（再描画）します。これにより、データと画面の表示が常に同期されます。

ここで注意したいのが、`setState`（状態を更新する関数）を呼び出しても、**現在のレンダー中ではstateの値はすぐに更新されない**という点です。次回レンダー時に初めて、更新された値が反映されます。

また、同じコールバック内で連続してstateを更新したい場合、そのまま値を渡すと意図した結果になりません。
...
このような場合、更新関数に**関数を渡す**ことで「stateの値に対してこう変更してね」と指示でき、複雑な更新が可能になります。
...
なお、Reactはパフォーマンス最適化のため、これらの連続した`setState`を「バッチ処理」として一括して行います。コールバック内の処理が全て終わってからstateを計算し、1回だけレンダーを発生させます（`setState`の都度レンダーが発生するわけではありません）。
...
- **一緒に変更されるstateはまとめる:** 常に同時に更新されるデータ（例えばマウスの座標`x`と`y`など）は、個別のstateにするのではなく、1つのオブジェクトにまとめます（`{ x: 0, y: 0 }`）。
- **計算できる情報はstateにしない:** パフォーマンスや一貫性の観点から、既存の`props`や`state`から計算で求められるものはstateとして持たないようにします。例えば、`firstName`と`lastName`があるなら、`fullName`はレンダリング時に結合して作ればよく、stateにする必要はありません。重複したstateを持ったり、矛盾した状態になるのを防ぐことができます。
- **深いネストを避ける:** 更新が複雑になるため、stateが深くネストされた構造になるのは極力避けます。

#### reducerを使用する

ショッピングカート機能のように、「アイテムの追加」「削除」「数量変更」など、複雑なstate更新が複数箇所で行われる場合に便利なのが`useReducer`です。一言で言うと、高度なstate更新のための機能です。
...
コンポーネントのツリーが深く、リフトアップしたstateを子孫コンポーネントの奥深くまでバケツリレーのように`props`で渡していくのが辛い場合、Contextを使用します。Contextを使えば、中間のコンポーネントを飛ばして、必要なコンポーネントに直接データを提供することができます。
