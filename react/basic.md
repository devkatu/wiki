...
なお、**stateの更新は、state更新関数のみで行い、`state = true`のような更新はしてはいけません。**これはreact側でstateの更新を検出できなくなってしまうためです。

```jsx
<button
  onClick={() => {
    setIsVisible(!isVisible);  // stateを更新
...
のように、**「前回state値を使って、次にセットしたいstate値を返す関数」**を渡してあげます。

**これは「今のstate値」に対する変更を加えたい場合**に使用します。
...
※今の値に対しての更新が必要な場合はこっちのほうがいい旨書く
プラス1する、配列に対して変更を加える、反転する、など
...
※ただしスプレッド構文は浅いコピー（shallow copy）であり、配列やオブジェクトの最上位階層のみをコピーするため、深くネストされたオブジェクトの更新には注意が必要です。複雑な場合は`Immer`といったライブラリを利用して、変更したいプロパティのみを指定して更新することもあります）
