...
Reactの標準的な機能を超えて、React外のシステムとやり取りしたい場合、`useEffect`、`useRef`等を使用することがあります。

### useEffect

`useEffect`は、コンポーネントがレンダーされたタイミングでUIを作る以外の何らかの処理(副作用)を行いたい場合に使用します。中でも、主に外部システムとの同期を行う事を目的として用意されています。

※副作用とは、コンポーネントの「見た目を作る」以外の作業を言います。例えばAPIからデータを取得する、タイマーをセットする、DOMを手動操作するなどです。

以降、このレンダーによって発生させる副作用のことを**エフェクト**と呼びます。

#### 主な用途

- **外部APIからのデータ取得**: 画面が表示された時に、サーバーからデータを取ってきてstateにセットする
...
- **ブラウザAPIの操作**: document.title を書き換えたり、ローカルストレージへの保存、ログの送信、要素のフォーカスなどです。

ただし、レンダー後に操作したい！からといって、次のような用途では使用しません。
...
// コンポーネント内にてuseEffect実行
useEffect(() => {
  // レンダリング後に実行したい処理(セットアップ処理)
  return () => {
    // クリーンアップ処理
  };
}, [依存変数, 依存変数, ...]);
...
**★コンポーネント関数のトップレベルで`useEffect`を呼び出します**  
...
このクリーンアップ処理は次のような時に呼ばれます。

- 再レンダリングが発生したとき  
  →一度クリーンアップ処理を行った後に、再度エフェクト(セットアップ処理)が実行されます
- コンポーネントが削除されたとき

レンダリング時に`useEffect`内で実行した処理を打ち消す場合に必須です。例えばサーバーとの接続を解除せずにいた場合、メモリリークとなり、そのコンポーネントがマウントされる都度動作が重くなってしまいます。

- サーバーとの接続の解除
- イベントリスナー登録の解除
- セットしたタイマーの解除

**★第二引数にはエフェクト実行を制御する為の依存変数の配列を渡します**  
`useEffect`処理の中で使用している、**props、state、コンポーネント本体で宣言した変数・関数**を配列として渡します。

これらは再レンダー時に変化する可能性のある値であり、これらが再レンダー時に変化している場合はエフェクトの実行が必要と判断します。これらはReactではリアクティブな変数とも言われます。

指定した変数が前回レンダーと今回レンダーで同じ(`Object.is()`を用いて判定している)場合はエフェクトの実行はスキップされます。

よって、エフェクトの実行タイミングは、第二引数によって次のようになります。

- 第二引数の依存配列がどれか一つでも変化すると、その再レンダー後に実行
- 第二引数に空の配列 `[]` を渡すと、初回レンダー後のみ実行

また、あまり使用用途はありませんが、第二引数を省略すると毎回のレンダー後に実行されるようになります。

#### エフェクトのライフサイクル

他のオブジェクト指向言語を経験していたり、クラスコンポーネント時代のReactを経験していたりする方は、コンポーネントのライフサイクルのタイミングでエフェクトが実行されるのでは？と思うかもしれません。

コンポーネントのライフサイクルは次のとおりです。

- マウント　・・・　コンポーネントが画面に追加された時
- アップデート　・・・　state等によりコンポーネントが更新された時
- アンマウント　・・・　画面から削除された時

エフェクトで意識するべきなのは次の２つです。

- **エフェクトの開始(セットアップ)**
- **エフェクトの終了(クリーンアップ)**

エフェクト処理でチャットサーバーに接続する場合を考えます。

エフェクト開始時にはチャットサーバーに接続したいので下記の通りです。

```javascript
useEffect(() => {
  // roomIdで指定されたチャットサーバーに接続する
  const connection = createConnection(serverUrl, roomId);
  connection.connect();
},[roomId])
```

エフェクト終了時にはチャットサーバーとの接続を解除しなければなりません。

```javascript{5-8}
useEffect(() => {
  // roomIdで指定されたチャットサーバーに接続する(エフェクト開始)
  const connection = createConnection(serverUrl, roomId);
  connection.connect();
  return () => {
    // チャットサーバーとの接続を解除する(エフェクト終了)
    connection.disconnect();
  };
},[roomId])
```

これらのエフェクト開始・終了時の処理が、実際のコンポーネントのライフサイクル時にどのように動くのか見てみましょう。

**★コンポーネントのマウント時**  
エフェクトの開始処理が走り、チャットサーバーへ接続されます。
...
**★コンポーネントのアップデート時**  
例えば新しい`roomId`を受け取ってアップデートされた場合、エフェクトの終了処理が走ったあと、再度エフェクトの開始処理が走ります。
...
開発環境でのみ、コンポーネントのマウント時に、次のようにエフェクトが実行されます。

1. エフェクト開始
2. エフェクト終了(開発環境のみ)
3. エフェクト開始(開発環境のみ)

開発環境のときのみ、2・3項の処理が余分に走ります。これはバグではなく、**クリーンアップ処理が必要なパターンの記述漏れを発見しやすくするため**です。クリーンアップの処理がきちんと記述されていて、再マウントされても正常に動作するコードとなっているかを早期に発見するための仕組みです。

#### 実装の落とし穴

エフェクトを実装するにあたり、次のようなパターンが出てくることがあります。

**★目的の違うエフェクトを一つにまとめない**

エフェクトで、「ページのタイトル更新」と「アクセスログの送信」という処理をしたい場合を考えます。

```javascript
function ProductPage({ productId, pageTitle, category }) {
  
  useEffect(() => {
    // 目的A：ブラウザのタブ名を更新する（SEO・UI用）
    document.title = pageTitle;

    // 目的B：サーバーにアクセスログを送信する（分析用）
    api.sendAnalytics(productId, category);

    // 依存配列に両方の変数を入れる
  }, [pageTitle, productId, category]); 
...
このコードでは、何らかの理由で、`pageTitle`だけが更新された場合でも、エフェクト全体が実行されてしまい、`productId`や`category`に変化が無くても、無駄にサーバーにアクセスログを送信してしまいます。

次のように、目的別に「何に反応し、何を実行するのか」を整理してエフェクトを書くのがいいでしょう。

```javascript
function ProductPage({ productId, pageTitle, category }) {

  // Effect 1：タイトルの同期だけを担当
  useEffect(() => {
    document.title = pageTitle;
  }, [pageTitle]); // pageTitleが変わった時だけ動く

  // Effect 2：ログ送信だけを担当
  useEffect(() => {
    api.sendAnalytics(productId, category);
  }, [productId, category]); // 商品IDが変わった時（＝別の商品を表示した時）だけ動く

  // ...
}
```

**★エフェクト内で依存値の最新の値を読み取りたい(でも再実行はさせたくない)**

エフェクト内の処理で、依存値の最新の値を使った処理をしたい・・・けどその依存値が更新されても再実行をさせたい訳ではない。という場面が出てきます。

次のコードは、カートに商品が入った時に、現在のテーマ（ダーク/ライト）に合わせて通知を出す、というものです。
...
function ProductCart({ items, theme }) {
  ...
  useEffect(() => {
    // 目的：商品が増えたら通知を出す
    if (items.length > 0) {
      showNotification(`カートに${items.length}個の商品があります`, theme);
    }

    // themeも使っているので、依存配列に入れざるを得ない
  }, [items, theme]); 
...
商品をカートに入れると、カートに入っている商品の個数の通知が表示されるものです。

が、通知の関数`showNotification`に`theme`(ダーク/ライトの切替)を使用しており、これは依存配列に入れざるを得ません。

しかし、`theme`を切り替えたからと言って、エフェクトを実行したい訳ではありません。
...
function ProductCart({ items, theme }) {
  ...
  // 非リアクティブなイベントとして切り出す
  // この中は「常に最新のtheme」が見えるが、これ自体はEffectを動かさない
  const onItemsUpdated = useEffectEvent((count) => {
    showNotification(`カートに${count}個の商品があります`, theme);
  });

  useEffect(() => {
    if (items.length > 0) {
      onItemsUpdated(items.length);
    }
    // 依存配列は「items」だけ！ themeが変わってもこのEffectは動かない
  }, [items]); 
...
**★依存配列中に、コンポーネントで定義したオブジェクト等(参照型)をそのまま入れない**
...
**★refの値をレンダリングに使用しない**  
Reactではrefはレンダリングのトリガに使用しないため、refの変化したタイミングは把握していません。refをレンダリングに使用すると、データは更新されるがUIは更新されずにアンマッチになってしまいます。

レンダリングに値を使用したい場合はstateを使用します。
...
Reactにおいて、パフォーマンスを向上するためには、無駄なレンダリングを防いだり、コンポーネントでの重い計算を繰り返す事を避けたりする為には、キャッシュを使うことが重要になってきます。これらは**メモ化**とも呼ばれます。

★**ReactCompiler**を導入すると、ここで紹介する`memo`、`useCallback`、`useMemo`が全て含まれています。

### memo

Reactの仕様上、再レンダリングが起こるタイミングには次のようなものがあります。

- 該当のコンポーネントのstateが更新された
- 親コンポーネントが再レンダリングされた
- useContextを使っているときに、そのデータが更新された

しかしこのうち、**親コンポーネントが再レンダリングされた**場合でも、必ずしも子コンポーネントの再レンダリングが必要とは限りません。propsが前回と変化していなければ、子コンポーネントは同じ見た目のハズなので再レンダリングする必要はありません。

じゃあ**親コンポーネントが再レンダーされても、propsが変化していなければ再レンダーしないコンポーネントがあればいいよね**、というときに使えるのが`memo`です。※`memo` はフックではなく、コンポーネントをラップして返す高階コンポーネント（HOC）というものです。

これにより、コンポーネントのレンダリング結果をメモ化することができ、不要な再レンダリングをスキップできます。

#### 使い方

使い方は非常に簡単です。

**メモ化したいコンポーネントを`memo`で包んであげるだけです。**
...
// メモ化したいコンポーネントをmemoでラップすると、返値を元のコンポーネントの代わりに使用
const MemoizedComponent = memo(function MyComponent(props) {
  // ...
});
```

たったこれだけで、受取っているpropsが変化していなければ再レンダリングしないコンポーネントになります。

#### 実装の落とし穴

但し、`memo`の「propsが前回値と変化していないか」のチェックは`Object.is()`を使ったものになります。**これは参照が全く同一か**、もチェックしています。つまり、**propsに再レンダー時に新しく作成した配列、オブジェクト、関数を渡していると、再レンダー前と値は同じでも、参照は違うので結局再レンダリングが発生**することになります。

これを解決するためには、後述する`useCallback`や`useMemo`と組み合わせて、propsとして渡す配列、オブジェクト、関数もメモ化する必要があります。
...
通常、コンポーネント内でイベントハンドラ等に使用する関数は、コンポーネントが再レンダリングされる度に新たに生成されますが、`useCallback`を使用すると、これをメモ化することが可能です。

これを使えば、再レンダリングされても**常に同じ参照の関数**をキープしてくれるようになります。
これを`memo`で対策した子コンポーネントに渡すことで、不要な再レンダリングを防ぐことができます。
...
**★コンポーネント関数のトップレベルで`useCallback`を呼び出します**  
フックはトップレベルで呼び出します。

**★第一引数にはメモ化したい関数を渡します**  
既存のイベントハンドラがあればそのまま渡します。

**★第二引数にはエフェクト実行を制御する為の依存変数の配列を渡します**  
第一引数の関数内で使用している、**props、state、コンポーネント本体で宣言した変数・関数**を配列として渡します。

これらは再レンダー時に変化する可能性のある値であり、これらが再レンダー時に変化している場合はメモ化した関数の再計算を行います。

`useEffect`に渡す依存配列と同様の考え方になります。

**★`useCallback`の返り値がメモ化された関数となります**  
メモ化した関数は、そのまま子コンポーネントのpropsに渡します。

```jsx
<MyButton onClick={memoizedCallback}>
クリック
</MyButton>
```

よくある誤りとしてメモ化した関数をさらに即時関数を作ってその中で実行してしまうものがあります。これだと再レンダリング時に結局新しい関数が作られてしまうのでメモ化の効果が無くなります。
