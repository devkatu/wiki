...
#### JSX中で`class`や`for`などの予約語は使えない

HTMLの属性名で`class`や`for`等のJavaScriptの予約語がありますが、これらはJSX中であってもあくまでJavaScriptとして解釈されるため、そのまま使用はできません。
...
|for|htmlFor|<label> で使用。JSの for ループと重複を避けるため|
...
|style|style={{...}}|文字列ではなく「オブジェクト」で渡す（下記参照）|
...
JSXの中のテキストや、属性値等に変数等を使用する場合、

#### CSSプロパティ等は基本的にはキャメルケースになる

#### コンポーネント関数内に、外のコンポーネント関数をネストしない。
