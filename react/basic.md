...
※しかし本来、JavaScriptの関数そのものは状態を保存しておくことができません。コンポーネント関数はレンダリングの都度呼び出され、その中のローカル変数はレンダリングが終わると破棄されてしまいます。Reactでは、React本体側にコンポーネント専用の変数領域を用意してもらうことで、そこにデータを保存し、あたかも関数が状態を持っているように見せかけることができます。これはフックと呼ばれる仕組みで実現され、stateの他にも様々なフックがあります。
