...
次の例のようなコンポーネントのライフサイクルイベントが発生したときに何らかの副作用を発生させたくなることがあります。

- コンポーネントが初めて画面に表示された(マウント)
- コンポーネントが削除された(アンマウント)
- コンポーネントがレンダリングされた後

※副作用とは、コンポーネントの「見た目を作る」以外の作業を言います。例えばAPIからデータを取得する、タイマーをセットする、DOMを手動操作するなどです。

`useEffect`、`useLayoutEffect`フックを使用すればこれらのタイミングで任意の処理ができます。

### useEffect

コンポーネントの**レンダリング後**に、副作用を実行するために使用します。毎回処理を実行することも、実行するタイミングを制御することもできます。
...
**★第一引数に渡した関数の返り値は、コンポーネントが削除(アンマウント)されたタイミングで実行された関数です**  
クリーンアップ処理として関数を定義します。不要な場合は省略可能です
...
なお、この配列中にオブジェクト等をそのまま入れないようにしましょう。

オブジェクトをそのまま記述すると、レンダリングの都度作られてしまうので
