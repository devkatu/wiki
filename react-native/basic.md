...
- 多言語化するときは`expo-localization`とかを実装して、playconsoleでストアの掲載情報を国別に適宜入力、ﾘﾘｰｽのタブから、国/地域を選択してターゲットを設定する。英語(en)を実装しとけば欧米の大体の国いけるみたい

## 公開したアプリの更新

機能追加、バグ修正等したアプリを公開するときには・・・

- リリースするアプリをビルド。**バージョンコードの更新を忘れずに！**単純に表示するバージョン`expo.version`と、`expo.android.VersionCode`
- playconsoleで製品版のリリースを作成し、
  - ビルドされた.aabファイルをアップロード
  - 更新理由を入力
  - 審査で弾かれることもあるので随時対応
