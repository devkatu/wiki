...
dpで指定すると解像度の異なる端末でも見た目のサイズは同じくらいになるようになっている。なので一個の端末でデバッグしてもほかの端末でも同じくらいのサイズになる…はず

##### もうちょっと詳しく
解像度が異なる状態でpx指定すれば当然見た目のサイズは異なる。さらに解像度
は同じでも画面の幅が異なるものであればこれも見た目が異なる。
なので***1インチの中にどれだけのドットがあるか(dpi)***を考慮したdpとういう単位を作った。  
端末ごとにdpiが存在するが、ある程度のdpiの区分を設けて汎用密度とし、それぞれに倍率を設定。下のmdpiを基準として自分の端末のdpi倍率を掛けてdpi->pxへの変換が行われる。
